...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
			...
			$qC = "UPDATE ProfileImages SET coverName = '$sql_file' WHERE profileID = '$profileidprofile'";
			$queryCover = mysqli_query($db, $qC);

				if($queryCover){
					$uploadresponse = "The file ". basename( $_FILES["fileToUpload"]["name"]). " has been uploaded.";
				}
				else{
				$uploadresponse = "Sorry, there was an error assigning your cover image location." . $qC;
				}
...
<br>
	<div id="profileCoverContainer">
		...
		<img src="<?=$coversource . $profileid . '/' . $coverimgname; ?>" alt="test"/>
		</div>
	</div>
