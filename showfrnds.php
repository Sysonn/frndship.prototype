...
$("#frndinput").on("click", function() {
	alert('WHY 2');
	$.ajax({
		url: 'frnds.php', // you can also use form.attr('action') if url is dynamic
        type: POST, // POST or GET,
        data: form.serialize(),
		success: function(data) {
                 // handle with response
                 alert('This totally worked!');
				 $("#idiv").html(data);
...
<input id="frndinput" type="button" name="submit" style="position: absolute; left: -9999px;"></input>
          ...
          '</div>';
              ...
              while($search_fetch = mysqli_fetch_array($search_query,MYSQLI_ASSOC)){
                ...
                echo
                  ...
                  '<a href='. $searchurl .' style=" display: block; text-decoration: none; color: inherit; height: 80px;">'.
                  '<div id = "FrndBox" onClick = '. $searchload . ';>' .
                /* onclick = location.href='. $searchurl . ' */
                /* Image */
  							'<div style="float: left; padding-right: 15px;">'.
                                '<img src="'. $simgsource . $searchID . '/' . $simgname .'" style="width: 60px; height: 60px; border-radius: 5px;">' .
  							'</div>'.
                ...
                '<div style="height: 25px; width: 500px;">'.
                  ...
                  elseif($searchfrndVal == '1' && $searchfrndReq == '1' ){
                    ...
                    border-width: 1px;
                    z-index: 2;
                    "><img src="media/acceptedfrnd.png" style="width: inherit; height: inherit;"></img></div>'.  '</div>';
                  }
                  ...
                  elseif($searchfrndVal == '0' && $searchfrndReq == '1'){
                    ...
                    border-width: 1px;
                    z-index: 2;
                    "><img src="media/sentfrnd.png" style="width: inherit; height: inherit;"></img></div>'.  '</div>';
                  }
                  elseif($searchprofileVal == '0' && $searchprofileReq == '1'){
                    echo
                    ...
                    border-width: 1px;
                    z-index: 2;
                    "><img src="media/addfrnd.png" style="width: inherit; height: inherit;"></img></div>'.  '</div>';
                  }
                  else
                  {
                    echo
...
while($fetchprofile = mysqli_fetch_array($profile_frnd_query,MYSQLI_ASSOC)){
  ...
  echo
      '<div id = "FrndBox" onClick = '. $frndload . ';>' .
        '<div style="float: left; padding-right: 15px;">'.
          '<img src="'. $frndprofilesource . $fetchprofileID . '/' . $frndprofileimgname .'" style="width: 60px; height: 60px; border-radius: 5px;">' .
        '</div>'.
        '<div style="
        float: left;
        color: #ED4A4A;
...
while($fetchfrnd = mysqli_fetch_array($frnd_query,MYSQLI_ASSOC)){
  ...
  echo
      '<div id = "FrndBox" onClick = '. $frndload . ';>' .
        '<div style="float: left; padding-right: 15px;">'.
          '<img src="'. $frndprofilesource . $fetchfrndID . '/' . $frndprofileimgname .'" style="width: 60px; height: 60px; border-radius: 5px;">' .
        '</div>'.
        '<div style="
        float: left;
        color: #ED4A4A;
